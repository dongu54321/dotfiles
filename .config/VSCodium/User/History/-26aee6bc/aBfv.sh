...
query_backup() {
    ...
    sqlite3 "$MAPPING_DB" "SELECT file_id, random_name, original_piece FROM mapping WHERE original_piece LIKE '$(basename "$folder")-%.tar.zst.gpg.%'" | while read original random; do
        echo "Original: $original_piece -> Random: $random_name"
    done
}
            # backup_name TEXT,
            # original_piece TEXT,
