...
query_backup() {
    ...
    sqlite3 "$MAPPING_DB" "SELECT file_id, random_name, original_piece FROM mapping WHERE original_piece LIKE '$(basename "$folder")-%.tar.zst.gpg.%'" | while read original_piece random_name file_id; do
