...
query_backup() {
    ...
    sqlite3 "$MAPPING_DB" "SELECT file_id, random_name, original_piece FROM mapping WHERE backup_name LIKE '$(basename "$folder")'" | while read original_piece random_name file_id; do
