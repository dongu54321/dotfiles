...
#   ex: backup_folder sonarr | backup_folder /path/to/sonarr
#   recover_backup <backup_name> <output_directory>
#   tdl-upload()
#   Upload all *.tar.zst.gpg.* in current folder to chat with random_name
# backup_folder:
...
#   - Renames each split piece to a random 17-character alpha-numeric filename.
...
random_filename() {
    tr -dc 'A-Za-z0-9' </dev/urandom | head -c 17
}
...
query_backup() {
    ...
    #Try more specific like sonarr-20250330echo "or Use sqlite browser to browse the database"
    ...
    echo "Recover: recover_backup backup_name output_directory"
}
...
tdl-upload() {
    ...
    for original in *.tar.zst.*; do
        ...
        local backup_name
        #replace.tar.zst.gpg.* with nothing for backup_name
        backup_name=${original//.tar.zst.gpg.*/}
        mv "$original" "$rand_name"
        echo "Mapping: $backup_name - $(basename "$original") -> $rand_name"
        sqlite3 "$MAPPING_DB" "INSERT INTO mapping (backup_name, original_piece, random_name) VALUES ('$backup_name', '$(basename "$original")', '$rand_name');"
        random_files+=("$rand_name")
