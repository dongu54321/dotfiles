...
# Global variables
SPLIT_SIZE="170m"          # Split size for backup pieces
...
backup_folder() {
    ...
    tar -I zstd -cf - --exclude-caches --exclude-backups \
      --exclude='*cache*' --exclude='*Cache*' --exclude='*telegram*' -C "$(dirname "$folder") "$folder_basename" | \
