...
backup_folder() {
    ...
    tar -I 'zstd -7' -cf - --exclude-caches --exclude-backups --exclude='*cache*' --exclude='*Cache*' --exclude='*telegram*' \
        -C "$(dirname "$folder")" "$folder_basename" |
        gpg -o- -c --batch --passphrase "MoMo6789\!@#" |
...
query_backup() {
    ...
    local filename="$(basename "$folder")-$date_created.tar.gpg"

    echo "Original and Random Filenames:"
    sqlite3 "$DB" "SELECT original, random FROM filenames WHERE original LIKE '$(basename "$folder")-%.tar.gpg.%'" | while read original random; do
        echo "Original: $original -> Random: $random"
    done
}
# Example usage:
