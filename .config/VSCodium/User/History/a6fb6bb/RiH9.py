...
class NovelScraper:
    ...
    def scrape_chapter(self, base_url, novel_slug, chapter_num):
        ...
        print(f"    Scraping URL: {url}")  # Debug line
        ...
        try:
            ...
            if not title_tag:
                print(f"    Could not find title with selector: {config['title_selector']}")
                return False
            ...
            if not content_div:
                print(f"    Could not find content with selector: {config['content_selector']}")
                return False
