...
function nebula_add_host
    if [ (count $argv) -ne 4 ]
        echo "Usage: nebula_add_host hostname remote_hostend remote_dir_pathend" 1>&2; return;
    end
    ...
    set group $argv[3]
    set lanip $argv[4]
    echo "$hostn $nebulaip $group $lanip"
    if [ -d "cert/$group/$hostn" ]
        echo "WARM cert exist skipping !"
    else
        nebula-cert sign -name "$hostn" -ip "$nebulaip/12" -groups "$group"
        mkdir -p "cert/$group/$hostn"
        cp ca.crt "cert/$group/$hostn"
        cp config.yaml "cert/$group/$hostn"
        cp nebula.service "cert/$group/$hostn"
        mv "$hostn".crt "cert/$group/$hostn"/host.crt
        mv "$hostn".key "cert/$group/$hostn"/host.key
    end

    cp install.sh "cert/$group/$hostn"
    ...
    scp -r -i "$keys" "$HOME/scripts/nebula/cert/$group/$hostn" dragon@"$lanip":/home/dragon/;
    echo Dragon6789\^\&\*\( | ssh -tt -i "$keys" dragon@$lanip "cd ~/$hostn && bash ~/$hostn/install.sh"
end
...
function app-install
    ...
    set hostn $argv[1];
    set apps $argv[2];
    scp -rC "$apps" "$hostn:/home/dragon/momopod/appdata/";
    ssh "$hostn" "cd ~/momopod/appdata/$apps && bash up.sh"
end
...
nebula_add_host <hostn> <nebulaip> <group> <lanip> ex: nebula_add_host dragon000 172.16.0.100 dragon 192.168.1.45
app-install <hostn> <app>    ex: app-install dragon000 caddy
'
end
