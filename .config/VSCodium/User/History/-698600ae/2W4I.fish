...
function tarscp_from_remote
    if [ "$#" -ne 3 ]; then
        echo "Usage: tarscp_from_remote remote_hostend remote_dir_pathend local_dir_pathend" 1>&2;
        return;
    fi;
...
function nebula_add_host
    ...
    local FILE=$HOME/.ssh/$group.pub;
    local keys=$HOME/.ssh/$group;
    if [ -f "$FILE" ]; then
