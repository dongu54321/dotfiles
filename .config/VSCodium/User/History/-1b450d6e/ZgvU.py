...
PREFIXES_TO_REMOVE = [
    ...
    "Người dịch:", "Team dịch:", "Dịch & biên:", "Dich:", "—–"
]
    ...
    def get_novel_slug(self, url):
        ...
        if 'truyen.tangthuvien.vn' in domain:
            ...
            try:
                ...
                # Handle URLs with chapter part
                clean_url = re.sub(r'chuong-\d+/?$', '', url.rstrip('/'))
                parts = clean_url.split('/')
                return [p for p in parts if p][-1]
        elif 'truyenfullmoi.com' in domain:
            parts = url.split('/')
            # Extract novel name from URL like /mau-than-ta-la-than-tham/
            for part in parts:
                if part and not part.endswith('.html') and not part.startswith('chuong-'):
                    return part
        # Fallback: extract from cleaned URL
