...
#curl -s "https://www.duckdns.org/update?domains=momoin&token=6fec108e-f354-412d-8ed6-ae99e40e9209&ip=100.64.0.4&ipv6=$(ip -6 addr show dev "eno1" |grep 'scope global temporary dynamic'| sed -e 's!.*inet6 \([^ ]*\)\/.*$!\1!;t;d' | grep -v 'fe69:4b7c/64' | grep -v '^fc' | grep -v '^fd00' | grep -v '^fe80' | grep -v '^fdc' | head -1)&verbose=true"