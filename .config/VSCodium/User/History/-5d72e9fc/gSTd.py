...
def signal_handler(sig, frame, processing_files):
    ...
    for file_path in list(processing_files):
        ...
        temp_output_path = os.path.join(args.output_dir, relative_path.replace('.txt', '.mp3.tmp'))
        if os.path.exists(temp_output_path):
            os.remove(temp_output_path)
...
def main():
    ...
    signal.signal(signal.SIGINT, lambda sig, frame: signal_handler(sig, frame, processing_files))
