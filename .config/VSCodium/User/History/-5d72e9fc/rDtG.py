...
def main():
    ...
    for i in range(0, len(text_files), BATCH_SIZE):
        ...
        print(f"Processing batch {i//BATCH_SIZE + 1}/{(len(text_files) + BATCH_SIZE - 1)//BATCH_SIZE}")
        
        with concurrent.futures.ThreadPoolExecutor(max_workers=args.threads) as executor:
            futures = []
            for file_path in batch:
                futures.append(executor.submit(
                    process_file, file_path, args.base_dir, args.output_dir, progress_file, semaphore
                ))
            
            for j, future in enumerate(concurrent.futures.as_completed(futures)):
                result = future.result()
                print(f"Progress: {i+j+1}/{len(text_files)} files processed")
                if not result:
                    all_successful = False
                    print(f"Failed to process file: {batch[j]}")
