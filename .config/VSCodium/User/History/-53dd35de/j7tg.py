...
class SeleniumHelper:
	...
	def __init__(self, driverType=None, PROXY_PORT=9351):
		self.driverType = driverType
		if driverType == 'proxy':
			profile = webdriver.FirefoxProfile()
			firefox_options = webdriver.FirefoxOptions()
			firefox_options.headless = False #Headless
			...
			profile = webdriver.FirefoxProfile()
			firefox_options = webdriver.FirefoxOptions()
			firefox_options.headless = True #Headless
			# firefox_options.profile = profile
			# profile.set_preference( "network.proxy.type", 1 )
			# profile.set_preference( "network.proxy.socks", "127.0.0.1" )
			# profile.set_preference( "network.proxy.socks_port", PROXY_PORT)
	...
	def getElementFrom(self, fromObject, selector):
		try:
			return fromObject.find_element(by=By.CSS_SELECTOR, value=selector)
		except:
			return None

	def getElementsFrom(self, fromObject, selector):
		try:
			return fromObject.find_elements(by=By.CSS_SELECTOR, value=selector)
		except:
			return None
	...
	def clickId(self, Id):
		try:
			self.find_element(By.ID, Id).click()
			return True
		except:
			return False
	...
	def clickName(self, Name):
		try:
			self.find_element(By.NAME, Name).click()
			return True
		except:
			return False
