...
def get_conversations_from_cookie(request):
    cookie_data = request.cookies.get('conversations')
    if cookie_data:
        try:
...
def set_conversations_in_cookie(response, conversations):
    cookie_data = base64.b64encode(pickle.dumps(conversations)).decode('utf-8')
    response.set_cookie('conversations', cookie_data, max_age=31536000)  # 1 year
...
def get_conversations():
    conversations = get_conversations_from_cookie(request)
...
def create_conversation():
    ...
    response = make_response(jsonify(conversation))
    set_conversations_in_cookie(response, conversations)
...
def add_message(conversation_id):
    ...
    if message['sender'] == 'user' and len(conversations[conversation_id]['messages']) == 1:
        conversations[conversation_id]['title'] = message['content']
