...
async def convert_to_audio(text, output_path, max_retries=5, rate="+17%", volume="+17%", pitch="+7Hz", proxy=None):
...
def process_file(file_path, base_dir, output_dir, progress_file, processing_files, rate, volume, pitch, proxy, normalize):
    ...
    try:
        ...
        # Normalize text if option is enabled
        if normalize:
...
def main():
    ...
    parser.add_argument('--output_dir', help='Output directory for audio files (default: {{base_dir}}_audio)'.replace('{', '{{').replace('}', '}}'))
    parser.add_argument('--final_output', help='Final combined audio file (default: {{base_dir}}.mp3)'.replace('{', '{{').replace('}', '}}'))
    parser.add_argument('--threads', type=int, default=5, help='Number of threads for parallel processing')
    parser.add_argument('--rate', default='+17%', help='Speech rate (e.g., +0%%, -10%%, +20%%)')
    parser.add_argument('--volume', default='+17%', help='Speech volume (e.g., +0%%, -50%%, +100%%)')
    parser.add_argument('--pitch', default='+7Hz', help='Speech pitch (e.g., +0Hz, -10Hz, +50Hz)')
    parser.add_argument('--proxy', help='Proxy server (e.g., http://user:pass@host:port)')
    parser.add_argument('--normalize', action='store_true', help='Enable text normalization (disabled by default)')
    args = parser.parse_args()
    
    # Set default output_dir if not specified
    if not args.output_dir:
        args.output_dir = args.base_dir + '_audio'
    
    # Set default final_output if not specified
    if not args.final_output:
        # Extract just the directory name from base_dir
        base_dir_name = os.path.basename(os.path.normpath(args.base_dir))
        args.final_output = base_dir_name + '.mp3'
...
#
# 2. Enable text normalization:
#    python novel_to_audio.py novel_chapters --normalize
#
# 3. Specify custom TTS settings:
#    python novel_to_audio.py novel_chapters --rate "-10%" --volume "+20%" --pitch "+10Hz"
#
# 4. Use a proxy server:
#    python novel_to_audio.py novel_chapters --proxy "http://user:pass@host:port"
#
# 5. Specify custom output directory and final output file:
...
#
# 7. Process novels in different directory:
#    python novel_to_audio.py /path/to/novel_chapters
#
# 8. Resume interrupted processing:
