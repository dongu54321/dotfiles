...
async def main():
    browser = await uc.start(
        ...
        browser_args=['--ignore-certificate-errors', '--window-size=1200,800'],
    )
    ...
    await field.set_value(ip6)

    print('click apply button')
    applybtn = await tab.select("#btnApply_ex")
    await applybtn.click()
    await tab.sleep(10)
    tab = await browser.back()
    print('Logout')
    logout = await tab.find("Logout", best_match=True)
    await logout.click()
    await tab.close()
