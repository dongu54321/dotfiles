...
alias dothardreset='git -C ~/dotfiles pull; git -C ~/dotfiles reset --hard origin/main'
alias dotfetchstash='git -C ~/dotfiles stash; git -C ~/dotfiles pull origin main; git -C ~/dotfiles stash apply'
...
alias postat-cpu='podman stats --no-stream --format "table {{.Name}}\t{{.Container}}\t{{.MemUsage}}" | sort -k 3 -h '
alias postat-ram='podman stats --no-stream --format "table {{.Name}}\t{{.Container}}\t{{.MemUsage}}" | sort -k 4 -h '
